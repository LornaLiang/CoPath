...
function RecordsPage() {
  ...
  const dialogueItems = data.dialogues.map((dialogue) => ({ key: dialogue.dialogue_id, label: <div className="dialogue-collapse-label"><span><MessageOutlined /> {dialogue.user_message}</span><small>{dialogue.created_at}</small></div>, children: <div className="dialogue-record"><div><RobotOutlined /><p>{dialogue.ai_response}</p></div><Collapse ghost size="small" items={[{ key: 'signal', label: '结构化学习状态信号', children: <pre className="signal-code">{JSON.stringify(dialogue.extracted_signal, null, 2)}</pre> }]} /></div> }))
